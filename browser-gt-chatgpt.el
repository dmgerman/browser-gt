;;; browser-gt-chatgpt.el --- ChatGPT backend for browser-gt  -*- lexical-binding: t; -*-
...
;; ChatGPT-specific backend for browser-gt.  Registers the CHATGPT
...
(defvar browser-gt-chatgpt-dir "~/sync/chatgpt"
  "Directory where ChatGPT conversation org files are saved.")
;; browser-gt-pandoc-executable is defined in browser-gt.el (shared with -www).

;; ── HTML → Org conversion ─────────────────────────────────────────────────────

(defun browser-gt-chatgpt--min-heading-level (html)
...
(defun browser-gt-chatgpt--html-to-org (html media-dir)
...
Signals an error if pandoc is not found or exits non-zero."
  ...
  (with-temp-buffer
    (let* ((min-lvl (browser-gt-chatgpt--min-heading-level html))
           (shift   (if min-lvl (- 3 min-lvl) 0))
           (exit-code (call-process-region (browser-gt--strip-svg html) nil
...
(defun browser-gt-chatgpt--id (url)
...
(defun browser-gt-chatgpt--sanitize-title (title)
...
(defun browser-gt-chatgpt--find-existing-dir (root id)
...
(defun browser-gt-chatgpt--format-turn (turn conv-dir)
...
Headings are routed through `browser-gt--sanitize-org-meta'.  Pandoc output
...
path is page-controlled and DOES get `browser-gt--sanitize-org-body' so a
`* heading' or `:PROPERTIES:' line in the raw text cannot break out."
  (let* ((role      (plist-get turn :role))
         ...
         (rich-body (and html (not (string-empty-p html))
                         (condition-case err
                             (browser-gt-chatgpt--html-to-org html conv-dir)
                           (error
                            (browser-gt--warn "HTML conversion failed, using plain text: %s"
                                                 (error-message-string err))
                            nil))))
         (body      (string-trim
                     (or rich-body
                         (browser-gt--sanitize-org-body (or raw ""))))))
    (if (string= role "user")
        (let* ((lines      (split-string body "\n" t))
               (first-line (or (car lines) ""))
               (rest       (cdr lines))
               (heading    (browser-gt--sanitize-org-meta
...
(defun browser-gt-chatgpt--save-html (messages file)
...
(defun browser-gt-chatgpt--save (payload)
...
plus any extracted images.  Returns the path of the org file written."
  (let* ((url   (plist-get payload :url))
         ...
         (id    (or (browser-gt-chatgpt--id url)
                    (progn
                      (message "Could not extract ID from URL: %s" url)
                      "unknown")))
         (root  (expand-file-name browser-gt-chatgpt-dir))
         (conv-dir (or (browser-gt-chatgpt--find-existing-dir root id)
                       (expand-file-name
                        (format "%s-%s-%s"
                                (format-time-string "%Y%m%d-%H%M%S")
                                id
                                (browser-gt-chatgpt--sanitize-title title))
                        root)))
    ...
    (condition-case err
        (browser-gt-chatgpt--save-html turns html-file)
      (error
    ...
    (condition-case err
        (with-temp-file file
          ...
          (insert (format "* %s\n\n" (browser-gt--sanitize-org-meta title)))
          (dolist (turn turns)
            (insert (browser-gt-chatgpt--format-turn turn conv-dir))))
      (error
...
(browser-gt-register-handler "CHATGPT" #'browser-gt-chatgpt--handle-chatgpt)

(provide 'browser-gt-chatgpt)
...
;;; browser-gt-chatgpt.el ends here
