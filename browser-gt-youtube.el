;;; browser-gt-youtube.el --- YouTube backend for browser-gt  -*- lexical-binding: t; -*-
...
;; YouTube backend for browser-gt.  Registers two request handlers:
;;
...
;;   (setq browser-gt-youtube-api-key       "YOUR-API-KEY")
;;   (setq browser-gt-youtube-videos-file   "~/org/videos.org")
;;   (setq browser-gt-youtube-transcript-dir "~/sync/youtube")
;;
;; Payload (YOUTUBE):
...
(defconst browser-gt-youtube--id-regexp "[A-Za-z0-9_-]\\{11\\}"
...
(defun browser-gt-youtube--sanitize-title (title)
...
(defun browser-gt-youtube--fetch-oembed (url)
...
Returns the first item hash-table, or signals an error."
  (let ((video-id (browser-gt-youtube--video-id url)))
    ...
    (let* ((api-url (format "https://www.googleapis.com/youtube/v3/videos?part=snippet,contentDetails,statistics&id=%s&key=%s"
                            video-id browser-gt-youtube-api-key))
...
(defun browser-gt-youtube--duration-to-minutes (duration)
...
(defun browser-gt-youtube--timestamp-to-seconds (ts)
...
(defun browser-gt-youtube--convert-chapters (description url)
...
`browser-gt--escape-org-link-desc' so it cannot close the link's
     ...
     (mapcar (lambda (line)
               (if (string-match "^\\([0-9:]+\\)[ \t]+\\(.*\\)$" line)
                   (let* ((ts      (match-string 1 line))
                          (label   (browser-gt--escape-org-link-desc
                                    (match-string 2 line)))
                          (seconds (browser-gt-youtube--timestamp-to-seconds ts)))
...
(defun browser-gt-youtube--escape-template (s)
...
(defun browser-gt-youtube--resolve-title (title oembed api-info)
...
(defun browser-gt-youtube--build-entry (url title selection oembed api-info video-id)
...
`browser-gt--sanitize-org-meta', multi-line body (description, selection)
via `browser-gt--sanitize-org-body'.  URL goes into `:URL:' raw — it is
already validated to a YouTube watch URL by `browser-gt-youtube--video-id'."
  (let* ((title       (browser-gt-youtube--resolve-title title oembed api-info))
         (title-clean (browser-gt--sanitize-org-meta
         ...
         (duration    (browser-gt-youtube--duration-to-minutes
                       (and details (gethash "duration" details))))
         (date        (and snippet (gethash "publishedAt"          snippet)))
         (description (browser-gt-youtube--sanitize-description
                       (browser-gt-youtube--convert-chapters
         ...
         (sel-text    (if (and selection (not (string-empty-p selection)))
                          (concat "\n"
                                  (browser-gt--sanitize-org-body selection)
                                  "\n")
                        "")))
    (concat "* TODO " title-clean "\n"
            ":PROPERTIES:\n"
            ":URL: "      url "\n"
            ":LANG: "     (browser-gt--sanitize-org-meta (or language "")) "\n"
            ":CATEGORY: " (browser-gt--sanitize-org-meta (or category "")) "\n"
            ":LENGTH: "   (browser-gt--sanitize-org-meta duration) "\n"
            ":AUTHOR:  "  (browser-gt--sanitize-org-meta (or author "")) "\n"
            ":CHANNEL:  " (browser-gt--sanitize-org-meta (or author-url "")) "\n"
            ":PDATE: "    (browser-gt--sanitize-org-meta (or date "")) "\n"
            ":END:\n\n"
            ytline
            "\nThe *Description*:\n\n"
            description
...
(defun browser-gt-youtube--sanitize-description (desc)
  "Sanitize the chapter-converted YouTube description DESC for body splicing.
`browser-gt-youtube--convert-chapters' has already produced safe `[[...]]'
chapter links; non-chapter lines remain page-controlled, so pass them
through `browser-gt--sanitize-org-body' to defuse heading/drawer/keyword
injection without touching the chapter links."
  (mapconcat #'browser-gt--sanitize-org-body
             (split-string desc "\n")
             "\n"))

;; ── YOUTUBE handler ───────────────────────────────────────────────────────────

(defun browser-gt-youtube--handle-youtube (payload)
...
Schedules the capture and returns immediately (respond-fast-then-defer)."
  ...
  (browser-gt-defer #'browser-gt-youtube--capture payload)
  (browser-gt--ok "Video capture started"))

(defun browser-gt-youtube--capture (payload)
  "Capture a YouTube video from PAYLOAD into `browser-gt-youtube-videos-file'."
  (condition-case err
      (let ((url   (plist-get payload :url))
        ...
        (browser-gt-youtube--video-for-later url title text))
    (error
     (browser-gt--warn "youtube capture failed: %s"
                          (error-message-string err)))))

(defun browser-gt-youtube--refresh-inline-images ()
  "Refresh inline images when visiting `browser-gt-youtube-videos-file'.
Uses `org-link-preview-region' (org 9.7+) when available and falls
back to `org-display-inline-images' on older org versions."
  (when (string-match (regexp-quote (expand-file-name browser-gt-youtube-videos-file))
                      (or (buffer-file-name) ""))
    (cond
...
(defun browser-gt-youtube--video-for-later (url title selection)
  "Capture YouTube video URL into `browser-gt-youtube-videos-file'.
...
`org-capture'."
  (let* ((video-id   (browser-gt-youtube--video-id url))
         (api-info   (condition-case err
                         (browser-gt-youtube--fetch-api-info url)
                       (error
                        (browser-gt--warn "API fetch failed (continuing without): %s"
                                             (error-message-string err))
                        nil)))
         (oembed     (condition-case err
                         (browser-gt-youtube--fetch-oembed url)
                       (error
                        (browser-gt--warn "oEmbed fetch failed (continuing without): %s"
                                             (error-message-string err))
                        nil)))
         (entry-text (browser-gt-youtube--escape-template
                      (browser-gt-youtube--build-entry
                       url title selection oembed api-info video-id)))
         (org-capture-templates
          `(("v" "Video for later" entry
             (file ,browser-gt-youtube-videos-file)
             ,entry-text
             :empty-lines-after 1
             :empty-lines-before 1
             :immediate-finish t
             :after-finalize browser-gt-youtube--refresh-inline-images))))
    (org-capture nil "v")
    (let ((path (expand-file-name browser-gt-youtube-videos-file)))
      (kill-new path)
      (message "Video appended to %s (path copied to clipboard)" path))))

;; ── YOUTUBE_TRANSCRIPT handler ────────────────────────────────────────────────

(defun browser-gt-youtube--handle-transcript (payload)
...
\(respond-fast-then-defer)."
  ...
  (unless (plist-get payload :url)
    (error "Missing url in payload"))
  (browser-gt-defer #'browser-gt-youtube--transcript-download-and-save payload)
  (browser-gt--ok "Transcript download started"))
...
(defun browser-gt-youtube--transcript-vtt-time-to-seconds (ts)
...
(defun browser-gt-youtube--transcript-format-timestamp (ts)
...
(defun browser-gt-youtube--transcript-strip-vtt-tags (text)
  "Strip VTT/HTML markup tags (including word-level timing) from TEXT."
  (string-trim (replace-regexp-in-string "<[^>]+>" "" text)))

(defun browser-gt-youtube--transcript-cue-text (lines)
...
timing tags, join all non-blank lines."
  (let ((timing-line (seq-find (lambda (l) (string-match-p "<c>" l)) lines)))
    (browser-gt-youtube--transcript-strip-vtt-tags
     (if timing-line
         timing-line
       (string-join (seq-filter (lambda (l) (not (string-empty-p (string-trim l)))) lines)
                    " ")))))

(defun browser-gt-youtube--transcript-vtt-to-org (vtt-content url)
  ...
  (let* ((video-id (browser-gt-youtube--video-id url))
      ...
      (cond
       ...
       ((string-empty-p line)
        ...
        (when (and current-ts current-txt)
          (let ((text (browser-gt-youtube--transcript-cue-text (reverse current-txt))))
    ...
    (mapconcat
     (lambda (cue)
       ...
       ;; `browser-gt--sanitize-org-body' before splicing.
       (let* ((ts      (car cue))
              (text    (cdr cue))
              (seconds (browser-gt-youtube--transcript-vtt-time-to-seconds ts))
              (display (browser-gt-youtube--transcript-format-timestamp ts)))
         (format "[[%s&t=%ss][%s]] %s"
...
(defun browser-gt-youtube--transcript-find-existing-dir (root video-id)
...
validated by `browser-gt-youtube--video-id' before reaching this call,
...
(defun browser-gt-youtube--transcript-make-dir (payload info)
  "Create and return the per-transcript directory path for PAYLOAD and INFO."
  (let* ((url      (plist-get payload :url))
         (title    (or (plist-get payload :title)
                       (and info (gethash "title" info))
                       "youtube-transcript"))
         (video-id (or (browser-gt-youtube--video-id url)
                       (error "Refusing transcript: %s is not a recognized YouTube watch URL"
                              url)))
         (root     (expand-file-name browser-gt-youtube-transcript-dir))
         (conv-dir (or (browser-gt-youtube--transcript-find-existing-dir root video-id)
                       (expand-file-name
                        (format "%s-%s-%s"
                                (format-time-string "%Y%m%d-%H%M%S")
                                video-id
                                (browser-gt-youtube--sanitize-title title))
                        root))))
    (make-directory conv-dir t)
    conv-dir))

(defun browser-gt-youtube--transcript-write-stub (payload info lang)
...
code that turned out to have no captions."
  (condition-case err
      (let* ((url        (plist-get payload :url))
             ...
             (conv-dir   (browser-gt-youtube--transcript-make-dir payload info))
        ...
        (with-temp-file file
          ...
          (insert (browser-gt--make-link url "Open in YouTube"))
          (insert "\n\n")
          (insert (format "No transcript available (language: %s).\n"
                          (browser-gt--sanitize-org-meta lang)))
          (when avail-man
    ...
    (error
     (browser-gt--warn "could not write stub: %s"
                          (error-message-string err)))))

(defun browser-gt-youtube--transcript-download-and-save (payload)
...
If no transcript is available, writes a stub org file and logs to *Messages*."
  (condition-case err
      (let* ((url      (plist-get payload :url))
             ...
             (info     (browser-gt-youtube--transcript-get-info url))
             (lang     (or (gethash "language" info) "en"))
             (eff-lang (browser-gt-youtube--transcript-effective-lang info lang)))
        (if (not eff-lang)
            (progn
              (browser-gt-youtube--transcript-write-stub payload info lang)
              (message "No transcript for %s (lang: %s)" url lang))
          (let* ((video-id (or (browser-gt-youtube--video-id url) "unknown"))
                 (title    (or (and info (gethash "title" info)) title))
                 (conv-dir (browser-gt-youtube--transcript-make-dir payload info))
                 (basename (file-name-nondirectory conv-dir))
                 (org-file (expand-file-name (concat basename ".org") conv-dir))
                 (vtt-path (browser-gt-youtube--transcript-download-vtt
                            url eff-lang conv-dir video-id)))
            (unless vtt-path
              (error "Browser-gt-youtube: VTT file not found after download"))
            (let* ((vtt-dest    (expand-file-name (concat basename ".vtt") conv-dir))
                   ...
                   (oembed     (condition-case e
                                 ...
                                 (error
                                  (browser-gt--warn "oEmbed fetch failed (continuing without): %s"
                                                       (error-message-string e))
                                  nil)))
                   (title      (browser-gt-youtube--resolve-title title oembed api-info))
              ...
              (with-temp-file org-file
                ...
                (insert (format "#+youtube_lang: %s\n"
                                (browser-gt--sanitize-org-meta eff-lang)))
                (insert (format "#+created: %s\n\n" (format-time-string "%Y-%m-%d %H:%M:%S")))
                (insert entry)
                (insert "\n** Transcript\n\n")
                (insert (browser-gt-youtube--transcript-vtt-to-org vtt-content url))
                (insert "\n")))
            (browser-gt--maybe-raise payload)
            (kill-new org-file)
            (message "Transcript saved to %s (path copied to clipboard)" org-file))))
    (error
     (browser-gt--warn "transcript failed: %s" (error-message-string err)))))

;; ── Register handlers ────────────────────────────────────────────────────────

(browser-gt-register-handler "YOUTUBE"             #'browser-gt-youtube--handle-youtube)
(browser-gt-register-handler "YOUTUBE_TRANSCRIPT"  #'browser-gt-youtube--handle-transcript)

;; ── Emacs-side helpers ──────────────────────────────────────────────────────

(defun browser-gt-youtube--url-with-timestamp (url seconds)
...
;;;###autoload
(defun browser-gt-youtube-org-link (&optional client)
...
CLIENT, when non-nil, names the connected browser-gt client;
...
connected."
  ...
  (let* ((tab   (browser-gt--active-tab client))
    ...
    (unless (and url (string-match-p
                      ...
                      url))
      (user-error "Browser-gt: active tab is not on YouTube"))
    (let* ((code "(() => { const v = document.querySelector('video');
                            return v ? v.currentTime : null; })()")
           (seconds (browser-gt--eval-active code client)))
      (unless (numberp seconds)
        (user-error "Browser-gt: no video element on the active YouTube tab"))
      (let* ((ts-url  (browser-gt-youtube--url-with-timestamp url seconds))
             (ts-str  (browser-gt--format-time-hms seconds))
             (desc    (format "%s @ %s" title ts-str))
             (link    (browser-gt--make-link ts-url desc)))
        (if (called-interactively-p 'any)
            (progn (insert link) nil)
          link)))))

(provide 'browser-gt-youtube)
...
;;; browser-gt-youtube.el ends here
